...
def plot(ds, x, v, dx, mean_profile_components='all', include_x_mean=True,
    ...
    cax = g.fig.add_axes([box.xmin, box.ymin - cb_pad - cb_height, box.width, cb_height])
    ...
    lines_profile = da_flux_tot.plot(y='zt', ax=g.ax_marg_y,
                     hue="sampling", add_legend=add_profile_legend)

    # add a square marker for the mask
    mask_marker = 's'
    mask_markersize = 4
    for n, (line, sampling) in enumerate(zip(g.ax_marg_y.get_lines(), da_flux_tot.sampling.values)):
        if sampling == 'mask':
    ...
    Nobj_bin_counts, _, _= ds[x].plot.hist(bins=bins, histtype='step',
                                           ax=g.ax_marg_x, color=objects_line_color)
