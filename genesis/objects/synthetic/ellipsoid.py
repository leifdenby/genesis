...
def make_ellipsoid_mask(grid, a=2, b=4, c=1, theta=0.0, phi=0.0):
    ...
    return x_**2./a**2. + y_**2./b**2. + z_**2./c**2. <  1.0
...
def find_ellipsoid_scales(da_mask):
    I_func = lambda x, y, z, m: np.array([
        [np.sum(m*(y**2. + z**2.)), np.sum(m*x*y), np.sum(m*x*z)],
        [np.sum(m*y*x),  np.sum(m*(x**2. + z**2.)), np.sum(m*y*z)],
        [np.sum(m*z*x),  np.sum(m*z*y), np.sum(m*(y**2. + x**2.))],
    ])
    ...
    x_c = x_3d.where(m, other=0.).sum(dtype='float64')/mass
    y_c = y_3d.where(m, other=0.).sum(dtype='float64')/mass
    z_c = z_3d.where(m, other=0.).sum(dtype='float64')/mass
    ...
    a = np.sqrt((la[1] + la[2] - la[0])*5./2.*1./mass)
    b = np.sqrt((la[0] + la[2] - la[1])*5./2.*1./mass)
    c = np.sqrt((la[0] + la[1] - la[2])*5./2.*1./mass)

    def calc_angles(v):
        phi = np.arctan2(np.abs(v[1]), np.abs(v[0]))
        l_xy = np.sqrt(v[0]**2. + v[1]**2.)
...
def test_plot_shape_mask():
    ...
    a, b = lx/4., lx/2.
    ds['mask'] = ds.x_3d**2./a**2. + ds.y_3d**2./b**2. + ds.z_3d**2. <  1.0
...
def test_make_ellipsoid_mask():
    ...
    b = lx/2.
    c = a
    mask = make_ellipsoid_mask(grid=(ds.x_3d, ds.y_3d, ds.z_3d), theta=0,
        phi=0, a=a, b=b, c=c)
    ...
    mask = make_ellipsoid_mask(grid=(ds.x_3d, ds.y_3d, ds.z_3d),
        theta=45./180*3.14, phi=0, a=a, b=b, c=c)
    ds['mask_rot'] = (ds.x_3d.dims, mask)

    plot_shape(ds)

    mask = make_ellipsoid_mask(grid=(ds.x_3d, ds.y_3d, ds.z_3d), theta=0,
        phi=60./180*3.14, a=a, b=b, c=c)
    ds['mask_rot'] = (ds.x_3d.dims, mask)
