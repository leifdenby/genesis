...
def main(objects, object_id, var_name=None):
    ...
    fov = 60.
    cam = scene.cameras.ArcballCamera(parent=view.scene, fov=fov, name='Arcball', up='z')
    cam.distance = nz*5
    view.camera = cam

    xax = scene.Axis(pos=[[-0.5*nx, -0.5*ny], [0.5*nx, -0.5*ny]], tick_direction=(0, -1),
    ...
    yax = scene.Axis(pos=[[-0.5*nx, -0.5*ny], [-0.5*nx, 0.5*ny]], tick_direction=(-1, 0),
                     font_size=16, axis_color='b', tick_color='b', text_color='b',
                     parent=view.scene)
