...
def map_property_onto_objects(objects, object_file, property):
    ...
    properties_mapped = xr.full_like(
        objects, fill_value=np.nan, dtype=object_property.dtype
    )
...
if __name__ == "__main__":
    ...
    ds = map_property_onto_objects(
        objects=objects, object_file=object_file, property=args.property
    )

    out_filename = "{}.objects.{}.mapped.{}.nc".format(
        base_name.replace("/", "__"), objects_mask, args.property
    )
