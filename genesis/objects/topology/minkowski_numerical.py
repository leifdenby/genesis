...
# shearing function
f_shear = lambda ls, h0: lambda h: ls/h0**2.*h**2


def len_fn_approx(h, ls, h0):
    "approximate distance along length"
    return np.sqrt(h**2. + f_shear(ls, h0)(h)**2.)


def len_fn(h, ls, h0):
    "numerically integrated distance along length"
    # ax^2 + bc + c
    dldh = lambda h_: np.sqrt(1. + (ls/h0**2.*2.*h_)**2.)
...
def make_plume_mask(grid, r0, h, shear_distance=0.0):
    ...
    """
    if h > grid.z.max():
        raise Exception("Grid too small to contain plume, please increase z coordinate max")

    ds = grid.copy()
    
    a = shear_distance/h**2
    ds['x_c'] = 0.0*ds.z + a*ds.z**2.
    ds['y_c'] = 0.0*ds.z
    ds['xy_dist'] = np.sqrt((ds.x - ds.x_c)**2. + (ds.y - ds.y_c)**2.)
    ds['r'] = r0 + 0.0*ds.z
...
def make_thermal_mask(grid, r0, h, z_offset=0.0, shear_distance=0.0):
    ...
    ds['mask'] = (ds.xy_dist/r0)**2. + (ds.z_dist/(h*s/2.))**2. < 1.0
    ds['type'] = 'plume'
...
def make_mask(h, l, dx, shape, l_shear, with_plot):
    ...
    if not os.path.exists(fn_plot):
        ...
        m.where(m, other=np.nan).plot(y='z', ax=ax, add_colorbar=False,
                                                cmap='Greys_r')
        plt.gca().set_aspect(1)
        plt.axis('off')
        plt.title("")
...
def mask_plot_example():
    ...
    ds_study = xr.Dataset(coords=dict(
        h=[1000.,],
        l=[2., 3., 4., 5., 6., 7., 8.],
        l_shear=[0., 500., 1000.], #[600., 400., 200., 0.],
        dx=[4.,],
        shape=['thermal',],
    ))
    ...
    fig, axes = plt.subplots(
        ...
        ncols=len(ds_study.l), sharex=True, sharey=True,
        figsize=(9,6),
    )
    ...
    def plot_mask(h, l, dx, shape, l_shear, with_plot=False):
        ...
        ax = axes[ii, jj]
        # only one object so can cast mask to int
        im = ds.sel(y=0, method='nearest').mask.plot.pcolormesh(ax=ax,
            y='z', cmap='Greys', linewidth=0, rasterized=True,
        )
...
def example2():
    ...
    try:
        for l_shear in list(ds_output.l_shear.values):
            ...
            l, = ds_.filamentarity.plot.line(marker='s', markersize=4, linestyle='--',
                                        label=r"$l_s$={}".format(format_length(l_shear)),
                                        ax=ax)
    except:
        ds_ = ds_output.swap_dims(dict(l='planarity'))
        ds_.filamentarity.plot.line(marker='s', markersize=4, linestyle='', 
                                    ax=ax, label=ds_.shape.values)
...
def example3():
    ...
    ds_study = xr.Dataset(coords=dict(
        h=[1000.,],
        l=[2., 3., 4., 5., 6., 7., 8.],
        l_shear=[0., 500., 1000., 1500.], #[600., 400., 200., 0.],
        dx=[4.,],
        shape=['thermal',],
    ))
    ds_output = apply_all(ds_study, calc_scales)
