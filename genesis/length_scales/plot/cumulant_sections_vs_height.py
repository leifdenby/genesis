...
def _load_data(dataset_names, var_names, z_max, z_step):
    ...
    for dn in dataset_names:
        ds = xr.open_mfdataset(["{}.{}.nc".format(dn, vn) for vn in var_names],
                              decode_times=False)
...
def main(datasets, var_names, est_method = WidthEstimationMethod.MASS_WEIGHTED):
    ...
    def sp(ds_, ax1, ax2):
        ...
        cumulant_analysis.covariance_direction_plot(ds_[v1], ds_[v2], ax=ax2, width_est_method=est_method)
        ax2.set_title('')
    ...
    for n, z_ in enumerate(tqdm(z)):
        ...
        axes[n,0].text(-1.0, 1.2, "z={}m".format(z_.values), transform=axes[n,0].transAxes)

    for n_d in range(len(datasets)):
        ax = axes[0,n_d*2]
        ax.text(1.7, 1.4, datasets[n_d].name, transform=ax.transAxes,
...
if __name__ == "__main__":
    ...
    argparser.add_argument('input', nargs='+',
                           help='input datasets, e.g. no_shear/rico.tn6')
    argparser.add_argument('--z_max', help='max height', default=700,
        type=float,)
    ...
    datasets = _load_data(args.input, var_names=set(var_names), z_max=args.z_max,
                          z_step=args.z_step)
