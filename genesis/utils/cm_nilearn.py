...
def _pigtailed_cmap(cmap, swap_order=("green", "red", "blue")):
    ...
    cdict = dict()
    cdict["green"] = [
        (0.5 * (1 - p), c1, c2) for (p, c1, c2) in reversed(orig_cdict[swap_order[0]])
    ]
    cdict["blue"] = [
        (0.5 * (1 - p), c1, c2) for (p, c1, c2) in reversed(orig_cdict[swap_order[1]])
    ]
    cdict["red"] = [
        (0.5 * (1 - p), c1, c2) for (p, c1, c2) in reversed(orig_cdict[swap_order[2]])
    ]
...
_cmaps_data = dict(
    ...
    purple_green=_pigtailed_cmap(_cm.Greens_r, swap_order=("red", "blue", "green")),
    purple_blue=_pigtailed_cmap(_cm.Blues_r, swap_order=("red", "blue", "green")),
    blue_orange=_pigtailed_cmap(_cm.Oranges_r, swap_order=("green", "red", "blue")),
...
def replace_inside(outer_cmap, inner_cmap, vmin, vmax):  # noqa
    ...
    cdict = dict()
    for this_cdict, cmap in [(outer_cdict, outer_cmap), (inner_cdict, inner_cmap)]:
    ...
    for c_index, color in enumerate(("red", "green", "blue")):
        ...
        color_lst.append((vmin, outer_cmap(vmin)[c_index], inner_cmap(vmin)[c_index]))
        ...
        color_lst.append((vmax, inner_cmap(vmax)[c_index], outer_cmap(vmax)[c_index]))
    ...
    return _colors.LinearSegmentedColormap(
        "%s_inside_%s" % (inner_cmap.name, outer_cmap.name), cdict, _cm.LUTSIZE
    )
