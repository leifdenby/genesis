...
coldpool_coarse.description = (
    "coarse coldpool detection using -0.1K from mean of theta_v at 100m"
)
...
moist_updrafts.description = (
    "regions of vertical moisture flux greater than 0.3 m/s kg/kg"
)
...
def boundary_layer_moist_updrafts(q_flux, z_max=650.0):
    z = q_flux.zt
    return np.logical_and(q_flux > 0.3e-3, z < z_max)


boundary_layer_moist_updrafts.description = (
    "regions in boundary layer of vertical moisture flux greater than 0.3 m/s kg/kg"
)
...
def coldpool_edge(
    ...
    ds["l_smoothing"] = (
        ...
        l_smoothing,
        dict(
            units="m", longname="smoothing lengthscale for smoothing coldpool perimeter"
        ),
    )

    ds["l_edge"] = ((), l_edge, dict(units="m", longname="with of coldpool edge mask"))
...
coldpool_edge_shear_direction_split.description = (
    "Coolpool edge split into up- and downshear direction"
)
...
rad_tracer_thermals.description = (
    r"radioactive tracer-based envelope ($\phi' > {num_std_div} \sigma(\phi)$)"
)
...
def rad_tracer_thermals_cb_crop(base_name, cvrxp, z_cb=650.0, num_std_div=1.0):
    ...
    """
    mask = rad_tracer_thermals(
        base_name=base_name, cvrxp=cvrxp, num_std_div=num_std_div
    )
    mask = mask.where(mask.zt < z_cb, other=0)
    return mask


rad_tracer_thermals_cb_crop.description = r"radioactive tracer-based envelope ($\phi' > {num_std_div} \sigma(\phi)$) clipped by cloud-base at z={z_cb}"
