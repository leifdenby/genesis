...
if (UCLALES_TRACKING_BIN_PATH is None
        or not os.path.exists(UCLALES_TRACKING_BIN_PATH)):
    HAS_TRACKING = False
else:
    HAS_TRACKING = True
...
def call(data_path, dataset_name, tn_start, tn_end,
    ...
    """
    if not HAS_TRACKING:
        raise Exception("""Couldn't find external cloud-tracking utility,
    ...
    try:
        ...
        raise Exception("There was a problem when calling the tracking "
                        "utility (errno={}): {} {}".format(
                            error_extra, return_code, ex
                            ))
    finally:
        os.chdir(old_dir)
