...
def _UCLALES_est_temperature(ds):
    ...
    try:
        ...
        arr_temperature = Pool().starmap(calc_temperature, zip(da_ql, da_pressure, da_thetal))
    except:
        arr_temperature = np.vectorize(calc_temperature)(q_l=da_ql, p=da_pressure, theta_l=da_thetal)

    da_temperature = xr.DataArray(
        arr_temperature, dims=da_pressure.dims,
...
def _plot_profiles(dataset_name, t_hrs_used):
    ...
    ds_ps.time.values = ds_ps.time.values/60./60.
    ...
    for n, t_ in enumerate(tqdm(t_hrs_used)):
        ...
        tephi = tephigram.Tephigram(fig=fig, subplotshape=subplotshape,
                                    default_lines='no_labels',
                                    y_range=(0., 15),
                                    x_range=(0., 60),
                                    )
        tephi.plot_temp(T=da_temperature-273.15, P=da_pressure/100.,
                marker=''
                )
        tephi.plot_RH(T=da_temperature-273.15, P=da_pressure/100.,
                      RH=da_rh, marker='')
